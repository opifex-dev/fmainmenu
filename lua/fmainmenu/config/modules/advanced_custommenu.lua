...
FMainMenu.ConfigModules[propertyCode].GeneratePanel = function(configSheet)
	...
	mainPropPanel.menuSetupEditorButton.DoClick = function(button)
		...
		-- frame setup
		MenuSetupEditor = vgui.Create( "fmainmenu_config_editor" )
		...
		local function updateCacheVisuals()
			...
			mainBPanel = vgui.Create("fmainmenu_config_editor_scrollpanel", MenuSetupEditor)
			...
			for i,button in ipairs(mainPropPanel.internalMenuSetup) do
				...
				-- button type
				local buttonTypeLabel = vgui.Create("fmainmenu_config_editor_label", buttonPanel)
				...
				buttonRemove.DoClick = function(button)
					FMainMenu.ConfigModulesHelper.doAdvancedConfirmationDialog(panelBlocker, function()
						-- Remove the button
						table.remove( mainPropPanel.internalMenuSetup, buttonPanel.bIndex )
						updateCacheVisuals()
				...
				buttonOrderUp.DoClick = function(button)
					if buttonPanel.bIndex > 1 then
						local temp = table.Copy(mainPropPanel.internalMenuSetup[buttonPanel.bIndex])
		...
		local function updateCachedTable(varTable)
			mainPropPanel.internalMenuSetup = table.Copy(varTable[2])
			updateCacheVisuals()
		...
		-- bottom toolbar
		local bottomPanel = vgui.Create("fmainmenu_config_editor_panel", MenuSetupEditor)
		...
		bottomPanelAddButton.DoClick = function(button)
			...
			local tempY = 0
			for _,bType in ipairs(table.GetKeys(buttonSetups)) do
		...
		panelBlocker =  vgui.Create("fmainmenu_config_editor_panel", MenuSetupEditor)
...
FMainMenu.ConfigModules[propertyCode].saveFunc = function()
	...
	parentPanel.lastRecVariable[2] = table.Copy(parentPanel.internalMenuSetup)
...
FMainMenu.ConfigModules[propertyCode].varFetch = function(receivedVarTable)
	...
	parentPanel.internalMenuSetup = table.Copy( receivedVarTable[2] )
end
